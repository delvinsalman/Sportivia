        ...
        <div className="flex min-h-0 flex-1 items-center justify-center px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-5">
          {!inLobby ? (
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex h-[min(520px,78svh)] w-full max-w-3xl flex-col overflow-hidden rounded-[24px] border-[3px] border-[#ed4245]/70 bg-[#121316]/95 shadow-[0_7px_0_#8f1e22] backdrop-blur-md"
            >
              {/* Top bar: sport + code */}
              ...
              {/* Fixed body — list scrolls inside; panel size never changes */}
              <div
                className={`grid min-h-0 flex-1 gap-2 px-3 py-2.5 sm:gap-3 sm:px-4 ${
                  bothHere
                    ? 'grid-rows-[auto_minmax(0,1fr)] sm:grid-cols-[200px_minmax(0,1fr)] sm:grid-rows-1'
                    : 'grid-rows-1'
                }`}
              >
                {/* Players */}
                <div className="flex shrink-0 flex-col gap-2 sm:min-h-0">
                    ...
                    {[0, 1].map(slot => {
                      ...
                      if (!p) {
                        return (
                          <div
                            key={`empty-${slot}`}
                            className="rounded-xl border-[2.5px] border-dashed border-[#2b2d31] bg-[#151619]/80 px-2.5 py-2"
                          >
                  ...
                  {!bothHere && (
                    <div className="mt-auto hidden rounded-xl border border-white/10 bg-[#151619]/80 px-3 py-4 text-center sm:block">
                      <p className="text-sm font-black text-[#f2f3f5]">Waiting for opponent</p>
                      <p className="mt-1 text-xs font-semibold text-[#6d6f78]">
                        Share the code. Stake unlocks when they join.
                      </p>
                    </div>
                  )}

                  {nothingToStake && (
                    <div className="mt-auto flex flex-col items-center justify-center rounded-xl border-[2.5px] border-[#f0b232]/40 bg-[#f0b232]/10 px-3 py-4 text-center">
                      <Ban className="mb-1.5 h-5 w-5 text-[#f0b232]" />
                      <p className="text-sm font-black text-[#f2f3f5]">Nothing to stake</p>
                      <p className="mt-1 text-[11px] font-semibold leading-snug text-[#949ba4]">
                        No {SPORT_LABEL[sport]} cards — hit Ready.
                      </p>
                    </div>
                  )}
                </div>

                {/* Stake — fixed scroll area */}
                {bothHere && !nothingToStake ? (
                  <div className="flex min-h-0 flex-col gap-2 border-t border-white/10 pt-2 sm:border-l sm:border-t-0 sm:pl-3 sm:pt-0">
                    ...
                    <label className="flex min-h-10 w-full shrink-0 items-center gap-2 rounded-xl border-[2.5px] border-[#2b2d31] bg-[#1a1b1f] px-2.5">
                    ...
                    <div className="flex shrink-0 flex-wrap gap-1">
                      {RARITY_FILTERS.map(filter => {
                        ...
                        const color = filter === 'all' ? '#f2f3f5' : RARITY_COLOR[filter];
                        return (
                          <button
                            key={filter}
                            type="button"
                            ...
                            className={`min-h-8 rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-wide transition-colors ${
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                      {filtered.length ? (
                          ...
                          {filtered.map(card => {
                            ...
                            return (
                              <button
                                ...
                                className={`flex min-h-11 w-full items-center justify-between rounded-xl border px-2.5 py-2 text-left transition-colors ${
                                  active
                        ...
                        <div className="flex h-full items-center justify-center px-3 py-5 text-center">
                          <div>
                    ...
                    <button
                      ...
                      className={`min-h-10 w-full shrink-0 rounded-xl border-[2.5px] py-2 text-xs font-black ${
                  ...
                  !bothHere && (
                    <div className="flex min-h-0 items-center justify-center rounded-xl border border-white/10 bg-[#151619]/60 px-4 text-center sm:hidden">
                      <div>
                        <p className="text-sm font-black text-[#f2f3f5]">Waiting for opponent</p>
                        <p className="mt-1 text-xs font-semibold text-[#6d6f78]">
