...
export function CharacterCardsScreen({
  ...
  const payment = useMemo(
    () => pendingUpgradePayment(profile, character, pending),
    [character, pending, profile],
  );
  const canConfirmUpgrades = queued > 0 && profile.coins >= payment.coinCost;
      ...
      <div className="relative z-10 flex h-full flex-col">
        <header className="relative z-20 flex shrink-0 items-center justify-between gap-3 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-2 sm:px-5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {(profile.freeUpgradeCredits ?? 0) > 0 && (
              <div
                className="inline-flex h-10 items-center gap-1.5 rounded-xl border-2 border-[#23a559]/70 bg-gradient-to-b from-[#1a2a1e] to-[#101612] px-2.5 shadow-[0_3px_0_#0a120d] sm:h-11"
                title="Free upgrades from Daily Spin"
              >
                <Sparkles className="h-3.5 w-3.5 text-[#4ade80]" strokeWidth={2.75} />
                <span className="font-mono text-sm font-black tabular-nums text-[#86efac]">
                  {profile.freeUpgradeCredits}
                </span>
                <span className="hidden text-[9px] font-black uppercase tracking-wider text-[#4ade80]/80 sm:inline">
                  Free
                </span>
              </div>
            )}
                  ...
                  <div className="overflow-hidden rounded-2xl border-[2.5px] border-[#23a559]/55 bg-gradient-to-b from-[#15241a] to-[#0f1411] shadow-[0_4px_0_#0a120d]">
                    <div className="flex items-center justify-between gap-2 border-b border-white/8 px-3 py-2">
                      ...
                      <div className="text-right">
                        {payment.creditsUsed > 0 && (
                          <p className="text-[9px] font-black uppercase tracking-wide text-[#86efac]">
                            {payment.creditsUsed} free
                            {payment.coinCost > 0 ? ' +' : ''}
                          </p>
                        )}
                        <p className="font-mono text-sm font-black tabular-nums text-[#f0b232]">
                          {payment.coinCost > 0
                            ? payment.coinCost.toLocaleString()
                            : payment.creditsUsed > 0
                              ? 'FREE'
                    ...
                    <div className="flex gap-2 px-3 pb-3">
                      ...
                      >
                        {canConfirmUpgrades
                          ? payment.coinCost > 0
                            ? `Confirm · ${payment.coinCost.toLocaleString()}`
                            : `Confirm · ${payment.creditsUsed} free`
                          : `Need ${(payment.coinCost - profile.coins).toLocaleString()}`}
                      </button>
                    </div>
                  </div>
                )}
